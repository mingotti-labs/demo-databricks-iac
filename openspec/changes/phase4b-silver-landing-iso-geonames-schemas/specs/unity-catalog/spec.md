...
`silver_landing_nsw_spatial`, `silver_landing_airroi`, `silver_landing_iso`,
`silver_landing_geonames`, `gold_analytics_gateway`,
`gold_integration_gateway`, and `gold_ai_gateway`. Silver Domain/Marts
schemas SHALL NOT be created yet, since domains are not yet defined — only
source-aligned Silver Landing schemas exist so far. No
...
- **WHEN** `databricks schemas list <catalog>` is run for each of `mdp_dev`,
  `mdp_tst`, `mdp_prd`
- **THEN** all twenty-nine schemas listed above are present in each
